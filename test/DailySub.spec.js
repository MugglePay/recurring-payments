  ...
  : describe("Daily Subscription Test", async function () {
      ...
      describe("UserA with daily subscription", async function () {
          describe("success", async function () {
              ...
              it("Relayer can't executePayment After subscription canceled", async function(){
                  const [userA, merchant, relayer] = await ethers.getSigners();
                  const { autoPayContract, mockTokenContract } = await loadFixture(deployContracts);
                  ...
                  const tx = autoPayContract.connect(relayer).executePayment(userA.address, merchant.address);
      ...
      describe("Relayer: executePayment", async function () {
          describe("success", async function () {
              const subscriptionCost = BigInt(10**18);
              const subscriptionPeriod = 2; 
              const frequency = 0; //daily

              it("Relayer should not be allowed to executePayment before period", async function (){
                  const [userA, merchant, relayer] = await ethers.getSigners();
                  const { autoPayContract, mockTokenContract } = await loadFixture(deployContracts);
              ...
              it("Relayer should not be allowed to executePayment after period :1day ", async function (){
                  const [userA, merchant, relayer] = await ethers.getSigners();
                  const { autoPayContract, mockTokenContract } = await loadFixture(deployContracts);
